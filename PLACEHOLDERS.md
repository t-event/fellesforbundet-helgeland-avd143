...
| 2 | **Navn/bilder på ansatte** | Data i [src/config.ts](src/config.ts) (`ANSATTE`, kommentert `PLASSHOLDER`), vist på [src/pages/tillitsvalgte.astro](src/pages/tillitsvalgte.astro) | Navn/bilder på de lønnede ansatte er ikke mottatt; vises som «Navn kommer». | Fyll inn navn/bilder i config.ts, fjern «Plassholder»-banneret over ansatt-lista. |
